...
int WIDTH = 100;
const int SENSOR_HISTORY_LENGTH = 400;
...
public:struct AccelerometerData {
 ...
 public:
    ...
    GLuint loadShader(GLenum shaderType, const std::string& pSource) {
        ...
        AccelerometerData update() {
        ALooper_pollAll(0, NULL, NULL, NULL);
        ASensorEvent event;
        //float a = SENSOR_FILTER_ALPHA;
        while (ASensorEventQueue_getEvents(accelerometerEventQueue, &event, 1) > 0) {
            sensorDataFilter.x = event.acceleration.x * 0.85;
            sensorDataFilter.y = event.acceleration.y * 0.85;
            sensorDataFilter.z = (event.acceleration.z - 9.7) * 0.85;
        }
    ...
    double getAxis_x() {
        ASensorEvent event;
        return event.acceleration.x - 9.8;
    }

    void render_REC() {
        glLineWidth(WIDTH/100);
        ...
        glUniform4f(uFragColorHandle, 0.6f, 0.6f, 0.6f, 1.0f);
        glDrawArrays(GL_LINE_STRIP, 0, SENSOR_HISTORY_LENGTH);

        sensorData[sensorDataIndex].y = sensorData[sensorDataIndex].y - 9;
        glVertexAttribPointer(vSensorValueHandle, 1, GL_FLOAT, GL_FALSE, sizeof(AccelerometerData),
                              &sensorData[sensorDataIndex].y);
        glUniform4f(uFragColorHandle, 0.3f, 0.3f, 0.3f, 1.0f);
        glDrawArrays(GL_LINE_STRIP, 0, SENSOR_HISTORY_LENGTH);

        glVertexAttribPointer(vSensorValueHandle, 1, GL_FLOAT, GL_FALSE, sizeof(AccelerometerData),
                              &sensorData[sensorDataIndex].z);
        glUniform4f(uFragColorHandle, 1.0f, 1.0f, 1.0f, 1.0f);
        glDrawArrays(GL_LINE_STRIP, 0, SENSOR_HISTORY_LENGTH);
    }
...
Java_com_android_accelerometergraph_AccelerometerGraphJNI_updateData(
        JNIEnv *env, jclass type) {
    (void)env;
    (void)type;
    sensorgraph::AccelerometerData data = gSensorGraph.update();
    double array [3];
    array[0] = data.x;
    array[1] = data.y;
    array[2] = data.z;

        jdoubleArray output = env->NewDoubleArray(3);

        env->SetDoubleArrayRegion( output, 0, 1, &array[0] );
        env->SetDoubleArrayRegion( output, 1, 1, &array[1] );
        env->SetDoubleArrayRegion( output, 2, 1, &array[2] );
...
Java_com_android_accelerometergraph_AccelerometerGraphJNI_getAxis(
        JNIEnv *env, jclass type) {
    (void)env;
    (void)type;
    return gSensorGraph.getAxis_x();
}
...
Java_com_android_accelerometergraph_AccelerometerGraphJNI_drawFrame_1REC(
        JNIEnv *env, jclass type) {
    (void)env;
    (void)type;
    gSensorGraph.render_REC();
}